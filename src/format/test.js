...
describe('format', function () {
  ...
  it('accepts a timestamp', function () {
    ...
    assert.equal(format(date, 'yyyy-MM-dd'), '2014-04-04')
  })
  ...
  describe('ordinal numbers', function () {
    it('ordinal day of an ordinal month', function () {
      ...
      assert.equal(result, '4th day of the 4th month of 1986')
    })
  ...
  it('era', function () {
    ...
    assert.equal(result, 'AD AD AD Anno Domini A')
  })
  ...
  describe('year', function () {
    describe('regular year', function () {
      it('works as expected', function () {
        ...
        assert.equal(result, '1986 1986th 86 1986 1986 01986')
      })
    ...
    describe('ISO week-numbering year', function () {
      it('works as expected', function () {
        ...
        assert.equal(result, '1986 1986 1986 1986 01986')
      })
  ...
  describe('quarter', function () {
    it('formatting quarter', function () {
      ...
      assert.equal(result, '2 2nd 02 Q2 2nd quarter 2')
    })
  ...
  describe('month', function () {
    it('formatting month', function () {
      ...
      assert.equal(result, '4 4th 04 Apr April A')
    })
  ...
  describe('week', function () {
    describe('local week of year', function () {
      it('works as expected', function () {
        ...
        assert.equal(result, '15 15th 15')
      })
      ...
      it('allows to specify `weekStartsOn` and `firstWeekContainsDate` in options', function () {
        ...
        assert.equal(result, '14 14th 14')
      })
    })
  ...
  describe('day', function () {
    ...
    describe('day of year', function () {
      it('works as expected', function () {
        ...
        assert.equal(result, '94 94th 94 094 00094')
      })
  ...
  describe('week day', function () {
    ...
    describe('ISO day of week', function () {
      it('works as expected', function () {
        ...
        assert.equal(result, '5 5th 05 Fri Friday F Fr')
      })
    ...
    describe('formatting day of week', function () {
      it('works as expected', function () {
        ...
        assert.equal(result, '6 6th 06 Fri Friday F Fr')
      })
  ...
  describe('day period and hour', function () {
    it('hour [1-12]', function () {
      ...
      assert.equal(result, '12 12th 12')
    })

    it('hour [0-23]', function () {
      var result = format(new Date(2018, 0 /* Jan */, 1, 0, 0, 0, 0), 'H Ho HH')
      assert.equal(result, '0 0th 00')
    })
    ...
    it('hour [1-24]', function () {
      var result = format(new Date(2018, 0 /* Jan */, 1, 0, 0, 0, 0), 'k ko kk')
      assert.equal(result, '24 24th 24')
    })

    describe('AM, PM', function () {
      it('works as expected', function () {
        var result = format(new Date(2018, 0 /* Jan */, 1, 0, 0, 0, 0), 'a aa aaa aaaa aaaaa')
        assert.equal(result, 'AM AM am a.m. a')
      })

      it('12 PM', function () {
        var date = new Date(1986, 3 /* Apr */, 4, 12, 0, 0, 900)
        assert.equal(format(date, 'h H K k a'), '12 12 0 12 PM')
      })

      it('12 AM', function () {
        var date = new Date(1986, 3 /* Apr */, 6, 0, 0, 0, 900)
        assert.equal(format(date, 'h H K k a'), '12 0 0 24 AM')
      })
    })
    ...
    describe('AM, PM, noon, midnight', function () {
      ...
      it('12 PM', function () {
        var date = new Date(1986, 3 /* Apr */, 4, 12, 0, 0, 900)
        assert.equal(format(date, 'b bb bbb bbbb bbbbb'), 'noon noon noon noon n')
      })

      it('12 AM', function () {
        var date = new Date(1986, 3 /* Apr */, 6, 0, 0, 0, 900)
        assert.equal(format(date, 'b bb bbb bbbb bbbbb'), 'midnight midnight midnight midnight mi')
      })
    })
    ...
    describe('flexible day periods', function () {
      ...
      it('12 PM', function () {
        var date = new Date(1986, 3 /* Apr */, 4, 12, 0, 0, 900)
        assert.equal(format(date, 'h B'), '12 in the afternoon')
      })

      it('5 PM', function () {
        var date = new Date(1986, 3 /* Apr */, 6, 17, 0, 0, 900)
        assert.equal(format(date, 'h B'), '5 in the evening')
      })

      it('12 AM', function () {
        var date = new Date(1986, 3 /* Apr */, 6, 0, 0, 0, 900)
        assert.equal(format(date, 'h B'), '12 at night')
      })

      it('4 AM', function () {
        var date = new Date(1986, 3 /* Apr */, 6, 4, 0, 0, 900)
        assert.equal(format(date, 'h B'), '4 in the morning')
      })
    })
  })
  ...
  describe('time zone', function () {
    ...
    it('Local date with specific non-location time zone', function () {
      ...
      assert.equal(result, dateAndTimeZoneAmericaNY)
    })
    ...
    it('handles quoted text next to a time zone token', function () {
      ...
      assert.equal(result, '04.04.1986 12:32 UTC+02:00')
    })
  ...
  describe('timestamp', function () {
    it('seconds timestamp', function () {
      ...
      assert.equal(result, secondsTimestamp)
    })
  ...
  describe('long format', function () {
    it('short date', function () {
      ...
      assert.equal(result, '04/04/1986')
    })

    it('medium date', function () {
      var result = format(date, 'PP')
      assert.equal(result, 'Apr 4, 1986')
    })

    it('long date', function () {
      var result = format(date, 'PPP')
      assert.equal(result, 'April 4th, 1986')
    })

    it('full date', function () {
      var result = format(date, 'PPPP')
      assert.equal(result, 'Friday, April 4th, 1986')
    })

    it('short time', function () {
      var result = format(date, 'p')
      assert.equal(result, '10:32 AM')
    })

    it('medium time', function () {
      var result = format(date, 'pp')
      assert.equal(result, '10:32:55 AM')
    })

    it('long time', function () {
      var result = format(date, 'ppp')
      assert.equal(result, '10:32:55 AM ' + timezoneGMTShort)
    })

    it('full time', function () {
      var result = format(date, 'pppp')
      assert.equal(result, '10:32:55 AM ' + timezoneGMT)
    })

    it('short date + time', function () {
      var result = format(date, 'Pp')
      assert.equal(result, '04/04/1986, 10:32 AM')
    })

    it('medium date + time', function () {
      var result = format(date, 'PPpp')
      assert.equal(result, 'Apr 4, 1986, 10:32:55 AM')
    })

    it('long date + time', function () {
      var result = format(date, 'PPPppp')
      assert.equal(result, 'April 4th, 1986 at 10:32:55 AM ' + timezoneGMTShort)
    })

    it('full date + time', function () {
      var result = format(date, 'PPPPpppp')
      assert.equal(result, 'Friday, April 4th, 1986 at 10:32:55 AM ' + timezoneGMT)
    })

    it('allows arbitrary combination of date and time', function () {
      var result = format(date, 'Ppppp')
      assert.equal(result, '04/04/1986, 10:32:55 AM ' + timezoneGMT)
    })
  })
  ...
  describe('edge cases', function () {
    ...
    it('handles dates before 100 AD', function () {
      ...
      assert.equal(format(initialDate, 'Y ww i'), '8 01 1')
    })
  })
  ...
  it('implicitly converts `formatString`', function () {
    ...
    // $ExpectedMistake
    assert.equal(format(date, formatString), '2014-04-04')
  })
